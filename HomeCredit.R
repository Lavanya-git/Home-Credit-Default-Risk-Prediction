...
#on Kaggle - many attributes have been removed for the purpose of this exercise
...
fs <- ggplot(loan.data, aes(x="", y=AMT_CREDIT, group=NAME_FAMILY_STATUS)) + 
  ...
  xlab("FAMILY STATUS")+ylab("AMT INCOME")
fs
...
#first single not married - change new field to 1 depending on flag family status, this can be used for logistic regression
...
#
# 6. Standardize AMT_INCOME_TOTAL using Z-Score Standardization. Calculate skewness to know if the attribute normally distributed?
...
# 7. Analyze AMT_INCOME_TOTAL for Outliers using the Z-Score values. 
...
# 8. Standardize AMT_CREDIT using Z-Score Standardization. Calculate skewness to know if the attribute normally distributed?
...
# 9. Analyze AMT_CREDIT for Outliers using the Z-Score values. 
...
amt_credit_frame
#11. Use binning to discretize AMT_TOTAL_INCOME into five bins named A through E with A being the lowest
# and E being the highest - name the new attribute CAT_AMT_TOTAL_INCOME into five bins named A through E with A being the lowest
...
table(loan.data$CAT_AMT_TOTAL_INCOME)
...
table(amt_income_frame$CAT_AMT_TOTAL_INCOME)
